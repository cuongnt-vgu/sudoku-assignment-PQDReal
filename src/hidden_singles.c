                    ...
                    if (p_board->p_rows[i][j]->candidates[c] == 1)
                    {
                        for (int u = 0; u < unit_size; u++)
                        {
                            ...
                            else
                            {
                                ...
                                for (int d = 0; d < unit_size; d++)
                                {
                                    if (!p_board->p_cols[i][u]->fixed && p_board->p_cols[i][u]->candidates[d] == 1 && d == c) count++;
                                }
                                if (count == 1)
                                {
                                    isHiddenSingle = true;
                        ...
                        if (isHiddenSingle)
                        {
                            ...
                            for (int a = 0; a < unit_size; a++)
                            {
                                for (int m = 0; m < unit_size; m++)
                                {
                                    ...
                                    if (p_board->p_cols[i][a]->candidates[m] == 1 && m == c && !p_board->p_cols[i][a]->fixed)
                                    {
                                        p_board->p_cols[i][a]->candidates[m] = 0;
                                        p_board->p_cols[i][a]->num_candidates -= 1;
                                    }
                                    if (p_board->p_boxes[i][a]->candidates[m] == 1 && m == c && !p_board->p_boxes[i][a]->fixed)
                                    {
