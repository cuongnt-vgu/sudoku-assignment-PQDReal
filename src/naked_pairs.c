            ...
            for (int j = i+1; j < BOARD_SIZE; j++)
            {
                if (p_cells[j]->num_candidates == 2)
                {
                    if (check_naked_pairs(p_cells, i, j))
                    {
                        ...
                        {
                            p_naked_pairs[(*p_np_np_nt_counter)++] = (NakedPairs){p_cells, i};
                            break;
                        }
                    }
                }
...
int naked_pairs(SudokuBoard *p_board)
{
    ...
    {
        find_naked_pairs(p_board->p_rows[i], p_naked_pairs, &np_nt_counter, 0);
        find_naked_pairs(p_board->p_cols[i], p_naked_pairs, &np_nt_counter, 0);
        find_naked_pairs(p_board->p_boxes[i], p_naked_pairs, &np_nt_counter, 1);
    }
    for (int i = 0; i < np_nt_counter; i++)
    {
